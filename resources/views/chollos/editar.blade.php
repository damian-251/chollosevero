...
<form action={{ route('chollos.actualizar', $chollo -> id) }} enctype="multipart/form-data" method="POST">
    ...
    <input type="text" name="categoria" placeholder="Categoría del chollo" class="form-control mb-2" 
    value="@foreach ($chollo->categorias as $categoria){{$categoria->nombre . ", "}}@endforeach " required>
